...
public class HediffComp_Immunisable : HediffComp_SeverityPerDay
{
	...
	public HediffCompProperties_Immunisable Props => (HediffCompProperties_Immunisable)props;
	...
	public Hediff Hediff => base.Pawn.health.hediffSet.hediffs.Find((Hediff x) => x.def == MyDefOf.TreatedInfection);
	...
	protected override float SeverityChangePerDay()
	{
		if (!FullyImmune)
		{
			if (Hediff != null)
			{
				Logr.Info("Found the def mothafuck");
				return Props.severityPerDayNotImmune * severityPerDayNotImmuneRandomFactor * -1f;
			}
			else
			{
